...
class TeleopState:
    ...
    def clip_values(self):
        ...
        self.x = np.clip(self.x, -0.08, 0.08)
        self.peak_height = np.clip(self.peak_height, 0.0, 0.2)

    def print_status(self):
        """ Prints the current teleoperation state to the console """
        os.system('clear')
        print(f"POS_X: {self.x:.2f}, PEAK_HEIGHT: {self.peak_height:.2f}")
