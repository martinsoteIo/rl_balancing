...
class XPos2DEnv:
    """
    Simulated environment for the Balancing Platform quadruped robot using the Genesis simulator.
    """
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, add_camera=False):
        ...
        self.ball_mass_range = torch.tensor(rand_cfg.get("ball_mass_range", [0.027, 0.027]), device=self.device, dtype=gs.tc_float)
        self.ball_friction_range = torch.tensor( rand_cfg.get("ball_friction_range", [0.0, 0.0]),device=self.device, dtype=gs.tc_float)
