...
class XPos3DEnv:
    ...
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, add_camera=False):
        ...
        model_path = "/home/admin/workspace/src/descriptions/balancing_platform_3d_description/urdf/robot.xml"
    ...
    def step(self, actions, is_train=True):
        ...
        # Apply clipping per motor with correct ranges and send to MuJoCo
        for i in range(self.num_envs):
            for j, dof_id in enumerate(self.motors_dof_idx):
                dof_min, dof_max = motor_ranges[j]
                clipped_pos = float(torch.clip(target_dof_pos[i, j], dof_min, dof_max).item())
        ...
        # (Opcional) orientación de la bola en mundo:
    ...
    def reset_idx(self, envs_idx):
        ...
        for i, name in enumerate(self.joint_names):
            ...
            # self.data.qpos[qadr] = self.default_dof_pos[i].item()
