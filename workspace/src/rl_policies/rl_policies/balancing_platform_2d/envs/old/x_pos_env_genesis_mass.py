...
class XPos2DEnv:
    ...
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, add_camera=False):
        ...
        self.robot: RigidEntity = self.scene.add_entity(
            gs.morphs.MJCF(
                file="/home/admin/workspace/src/descriptions/balancing_platform_2d_description/urdf/robot_bounce.xml",
        ...
        self.ball_mass_range = torch.tensor(rand_cfg.get("ball_mass_range", [0.001, 0.05]), device=self.device, dtype=gs.tc_float)
    ...
    def reset_idx(self, envs_idx):
        ...
        m_low = self.ball_mass_range[0]
        m_high = self.ball_mass_range[1]
        new_masses = gs_rand_float(m_low, m_high, (len(envs_idx),), self.device)
        self.ball_mass_current[envs_idx] = new_masses
        mass_shift = (self.ball_mass_current[envs_idx] - self.ball_nominal_mass).reshape(-1, 1)
        self.robot.set_mass_shift(
            mass_shift = mass_shift,
            links_idx_local = [self.ball.idx_local],
            envs_idx = envs_idx
        )
