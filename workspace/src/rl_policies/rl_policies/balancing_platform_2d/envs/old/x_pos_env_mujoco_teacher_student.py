...
class XPos2DEnv:
    ...
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, add_camera=False):
        ...
        model_path = "/home/admin/workspace/src/descriptions/balancing_platform_2d_description/urdf/robot_mujoco.xml"
    ...
    def step(self, actions, is_train=True):
        ...
        clipped_dof_pos = torch.clip(target_dof_pos, -1.57, 0.0)
        ...
        omega_body = self.data.cvel[self.platform_body_id, 3:6]  # ω en marco del cuerpo
        R = self.data.xmat[self.platform_body_id].reshape(3, 3)  # mundo <- cuerpo
    ...
    def _reward_imitate_teacher(self):
        # Ambos en espacio de motores [-1.5, 0.0]
        target_policy  = self.extras["control"]["target_dof_pos_policy"]  # [N,2]
        target_teacher = self.teacher_targets                              # [N,2]
        diff = target_policy - target_teacher
        s2 = (0.25 ** 2)   # sigma de imitación (ajústalo si hace falta)
        return torch.exp(- torch.sum(diff * diff, dim=1) / s2)